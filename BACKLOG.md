...
Landed: ``ecs/autobuild.py`` + ``BuildState.autobuild`` (persisted
``planets.autobuild`` column) + ``Auto`` button on the Colony screen
cycling Off → Balanced → Economic → Scientific → Militaristic. A new
``autobuild_tick`` runs before ``production_tick`` and queues the next
priority project on any player colony whose queue+current are empty.
Ships are intentionally skipped — autobuild only queues buildings, so
fleets stay an explicit decision.
